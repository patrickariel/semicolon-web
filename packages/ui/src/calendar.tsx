...
import { buttonVariants } from "@semicolon/ui/button";
import {
  Select,
  SelectContent,
  SelectItem,
        ...
        Dropdown: ({ ...props }) => (
          ...
          >
            <SelectTrigger
              className={cn(
                buttonVariants({ variant: "ghost" }),
                "h-7 w-fit space-x-1 border-0 py-2 pl-2 pr-1 font-medium [.is-between_&]:hidden [.is-end_&]:hidden [.is-start.is-end_&]:flex",
              )}
            >
            ...
            <SelectContent className="scrolling-auto max-h-250px min-w-[var(--radix-popper-anchor-width)] overflow-y-auto">
              {props.children &&
                React.Children.map(props.children, (child) => (
                  <SelectItem
                    value={(child as React.ReactElement).props?.value as string} // eslint-disable-line @typescript-eslint/no-unsafe-member-access
                    className="min-w-[var(--radix-popper-anchor-width)]"
                  >
                    {
                      (child as React.ReactElement).props?.children // eslint-disable-line @typescript-eslint/no-unsafe-member-access
                    }
                  </SelectItem>
                ))}
            </SelectContent>
